...
export const getAllHealthInsurancesApi = (page: number, status: string, scholastic: string, search: string) => {
  if (status === 'All') {
    return http.get(
      `api/health-insurance?Search=${search}&SortBy=Id&PageNumber=${page}&PageSize=7&IsDescending=true&Filter=${scholastic}`,
      {
    ...
    return http.get(
      `api/health-insurance?Search=${search}&SortBy=Id&PageNumber=${page}&PageSize=7&IsDescending=true&FilterStatus=True&Filter=${scholastic}`,
      {
        signal: controller.signal
      }
    )
    ...
    return http.get(
      `api/health-insurance?Search=${search}&SortBy=Id&PageNumber=${page}&PageSize=7&IsDescending=true&FilterStatus=False&Filter=${scholastic}`,
      {
        signal: controller.signal
      }
    )
